<template>
  <div class="min-h-screen bg-[#080d18] text-white">
    ...
    <div class="bg-gradient-to-b from-[#0d1b38] to-[#080d18] px-5 pt-12 pb-6">
      ...
      <p class="text-slate-500 text-xs mb-1">Step 2 of 3</p>
      <h1 class="text-2xl font-bold text-white">Tax Code</h1>
      <p class="text-slate-400 text-sm mt-1">Tax verification</p>
    </div>

    <!-- Form card -->
    <div class="bg-[#111827] rounded-2xl mx-4 mt-4 p-5 border border-[#1e2d44]">
      <p class="text-slate-400 text-sm mb-5">
        Enter your TAX verification code to proceed to the next step.
      </p>

      <form @submit.prevent="submit" class="space-y-4">
        <div>
          <label class="block text-xs text-slate-400 mb-1.5">Tax Code</label>
          <input
            v-model="code"
            type="password"
            required
        ...
        >
          {{ saving ? 'Verifying...' : 'Verify Tax Code' }}
        </button>
      </form>
    </div>
  </div>
...
const submit = async () => {
  ...
  saving.value = true
  try {
    const { data } = await client.post('/api/user/transfer-verify-tax.php', { tax_code: code.value })
