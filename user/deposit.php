...
if(isset($_POST['deposit'])) {
    ...
    $file = $_FILES['image'];
    $path = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $allowed = ['jpg', 'png', 'jpeg'];

    if (!in_array($path, $allowed)) {
        notify_alert('Only JPG/PNG images are allowed', 'danger', '3000', 'Close');
    } elseif (move_uploaded_file($file['tmp_name'], '../assets/deposit/' . time() . '.' . $path)) {
        $n = time() . '.' . $path;
        if ($acct_stat === 'hold') {
            ...
            if (true) {
                ...
                $email_message->send_to_both($email, $message, "[DEPOSIT] - $APP_NAME");
